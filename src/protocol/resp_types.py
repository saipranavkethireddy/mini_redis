...
class Store:
    ...
    def __init__(self):
        ...
        #Expiration registry - stores the expiration time of the keys
        self._expires: Dict[str,float] = {}

    #--------------------------
    #EXPIRATION AND TTL ENGINE
    #--------------------------

    def _is_expired(self, key: str) -> bool:
        """
        checks if a key has passes its expiration time. If expired,
        automatically deletes the key from memory.
        """
    ...
    def persist(self, key: str) -> int:
        """
        Removes the existing timeout on key, turning it into persistent key
        """ 
        if self._is_expired[key] or key not in self._data:
            return 0
        if key in self._expires:
    ...
    def get(self, key: str) -> Optional[str]:
        """Gets value of key. Returns None if key does not exist or is the key is of wrong type"""
    ...
    def delete(self, *keys: str) -> int:
        """Deletes one or more keys. Also returns the count of deleted keys."""
        deleted_count = 0
        for key in keys:
    ...
    def exists(self, *keys: str) -> int:
        """Returns count of existing non_expired keys."""
        count = 0
        for key in keys:
